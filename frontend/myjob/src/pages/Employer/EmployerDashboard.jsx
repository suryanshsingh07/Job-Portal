...
import { Plus, Briefcase, Users, Building2, TrendingUp, CheckCircle2, Heart } from "lucide-react";
    ...
    {(title || headerAction) && (
      <div className='flex items-center justify-between p-6 pb-4'>
        <div>
          ...
          {subtitle && (
            <h3 className='text-sm text-gray-500 mt-1'>{subtitle}</h3>
          )}
        </div>
        {headerAction}
      </div>
            ...
            <Card
              ...
              <div className='space-y-3'>
                {dashboardData?.data?.recentApplications
                  ?.slice(0, 3)
                  ?.map((data, index) => (
                    <ApplicationDashboardCard key={index} applicant={data?.applicant || ""} position={data?.job?.title || ""} time={moment(data?.updatedAt).fromNow()} />
                  ))}
              </div>
            </Card>
          </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
              {[
                ...
                },
                {
                  ...
                  icon: Users,
                  color: "bg-green-50 text-blue-700",
                  path: "/manage-job",
                },
                {
                  title: "Company Settings",
                  icon: Building2,
                  color: "bg-orange-50 text-blue-700",
                  path: "/company-profile",
                },
              ].map((action, index) => (
                <button
                  key={index}
                  className='flex items-center space-x-3 p-4 rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transiton-all duration-200 text-left cursor-pointer'
                  onClick={() => navigate(action.path)}>
                  <div className={`p-2 rounded-lg ${action.color}`}>
                    <action.icon className='h-5 w-5' />
                  </div>
                  <span className='font font-medium text-gray-900'>
                    {action.title}
                  </span>
                </button>
              ))}
