...
import {NAVIGATION_MENU, JOB_SEEKER_MENU} from '../../utils/data';
...
const DashboardLayout = ({activeMenu, children}) => {
    ...
    const menuItems = user?.role === 'employer' ? NAVIGATION_MENU : JOB_SEEKER_MENU;
  return (
    <div className='flex h-screen bg-gray-50'>
        ...
        <div className={`fixed inset-y-0 left-0 z-50 transition-transform duration-300 transform ${
            ...
            <div className='flex items-center h-16 border-b border-gray-200 pl-6'>
                {!sidebarCollapsed? (
                    ...
                    <span className='text-gray-900 font-bold text-xl'>My Job</span>
                    </Link>
                ):(
        ...
        {isMobile && sidebarOpen && (
            <div className='fixed inset-0 bg-opacity-25 z-40 backdrop-blur-sm' onClick={()=> setSidebarOpen(false)}/>
        )}
        <div className={`flex-1 flex flex-col transition-all duration-300 ${
            ...
            <header className='bg-white/80 backdrop-blur-sm border-b border-gray-200 h-16 flex items-center justify-between px-6 sticky top-0 z-30'>
                <div className='flex items-center space-x-4'>
                    ...
                    <div>
                        <h1 className='text-base font-semibold text-gray-900'>
                            Welcome back, {user?.name || user?.fullName?.split(" ")[0] || "User"}! 
                        </h1>
                        <p className='text-sm text-gray-500 hidden sm:block'>
                            {user?.role === 'employer' ? "Here's what's happening with your jobs today" : "Let's explore your latest career opportunities"}
                        </p>
                    </div>
                </div>
                <div className='flex items-center space-x-3'>
                    ...
                    avatar={user?.avatar || ""}
                    companyName={user?.name || user?.fullName || "Profile"}
                    email={user?.email || ""}
                    onLogout={logout}/>
                </div>
            </header>
